...
#define ROUNDED_INTERACTIONS_COUNT (INTERACTIONS_COUNT + INTERACTIONS_COUNT % 2)
	...
	// and magnitudes of force between the bodies for each interaction. For
	// the position_Deltas array, instead of using a one dimensional array
	// of structures that each contain the X, Y, and Z components for a
	// position delta, a two dimensional array is used instead which
	// consists of three arrays that each contain all of the X, Y, and Z
	// components for all of the position_Deltas. This allows for more
	// efficient loading of this data into SSE registers. Both of these
	// arrays are also set to contain ROUNDED_INTERACTIONS_COUNT elements to
	// simplify one of the following loops and to also keep the second and
	// third arrays in position_Deltas aligned properly.
	static alignas(
	    __m128d) double position_Deltas[3][ROUNDED_INTERACTIONS_COUNT],
	...
	for (intnative_t i = 0, k = 0; i < BODIES_COUNT - 1; ++i)
		for (intnative_t j = i + 1; j < BODIES_COUNT; ++j, ++k)
			for (intnative_t m = 0; m < 3; ++m)
				position_Deltas[m][k] = bodies[i].position[m] -
							bodies[j].position[m];

	// Calculate the magnitudes of force between the bodies for each
	// interaction. This loop processes two interactions at a time which is
	// why ROUNDED_INTERACTIONS_COUNT/2 iterations are done.
	for (intnative_t i = 0; i < ROUNDED_INTERACTIONS_COUNT / 2; ++i) {
		...
		// Doing square roots normally using double precision floating
		// point math can be quite time consuming so SSE's much faster
		// single precision reciprocal square root approximation
		// instruction is used as a starting point instead. The
		// precision isn't quite sufficient to get acceptable results so
		// two iterations of the Newton–Raphson method are done to
		...
		for (intnative_t j = 0; j < 2; ++j)
			// Normally the last four multiplications in this
			// equation would have to be done sequentially but by
			// placing the last multiplication in parentheses, a
			// compiler can then schedule that multiplication
			// earlier.
			distance_Reciprocal =
			    distance_Reciprocal * 1.5 -
		...
		// Typically this calculation would probably be done by using a
		// division by the cube of the distance (or similarly a
		// multiplication by the cube of its reciprocal) but for better
		// performance on modern computers it often will make sense to
		// do part of the calculation using a division by the
		// distance_Squared which was already calculated earlier.
		// Additionally this method is probably a little more accurate
		// due to less rounding as well.
		((__m128d *)magnitudes)[i] =
		    0.01 / distance_Squared * distance_Reciprocal;
	}

	// Use the calculated magnitudes of force to update the velocities for
	// all of the bodies.
	for (intnative_t i = 0, k = 0; i < BODIES_COUNT - 1; ++i)
		for (intnative_t j = i + 1; j < BODIES_COUNT; ++j, ++k) {
			// Precompute the products of the mass and magnitude
			// since it can be reused a couple times.
			const double i_mass_magnitude =
					 bodies[i].mass * magnitudes[k],
				     j_mass_magnitude =
					 bodies[j].mass * magnitudes[k];
			for (intnative_t m = 0; m < 3; ++m) {
				bodies[i].velocity[m] -=
	...
	// Use the updated velocities to update the positions for all of the
	// bodies.
...
static void output_Energy(body bodies[]) {
	...
	for (intnative_t i = 0; i < BODIES_COUNT; ++i) {
		...
		for (intnative_t j = i + 1; j < BODIES_COUNT; ++j) {
			...
			for (intnative_t m = 0; m < 3; ++m)
				position_Delta[m] = bodies[i].position[m] -
						    bodies[j].position[m];
